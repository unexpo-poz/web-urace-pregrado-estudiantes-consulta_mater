...
include_once ('inc/activaerror.php');
	...
	$conexion = new ODBC_Conn("CENTURA-DACE","c","c",true, 'busca_materias.log');
	$cSQL = "SELECT COUNT (status) FROM dace006 WHERE lapso='".$lapsoProceso."' ";
	$cSQL.= "AND exp_e='".$dp[3]."' AND status IN ('7','A','2','R','Y')";
	$conexion->ExecSQL($cSQL,__LINE__,true);
	//echo $cSQL;
	$cerrar = ($conexion->result[0][0] == 0) ? true:false;
	
	if ($cerrar){
		die('<script languaje=\"javacript\">alert("Lo siento, no tiene asignaturas para mostrar.\n\nPara mas informacion dirijase a URACE.");window.close();</script>');
	}
	
	/*$foto = $dp[2].".jpg";
	
	if (!file_exists($_SERVER['DOCUMENT_ROOT']."/img/fotos/".$foto)) {
		die('<script languaje=\"javacript\">alert("Lo siento, debe tener una fotografia registrada para mostrar su planilla de inscripcion.\n\nPara registrar su fotografia dirijase a la Unidad de Audiovisual.");window.close();</script>');
	}
*/
?>
            ...
            <tbody>
                <tr>
                    ...
                    <td style="width: 250px; padding-left: 20px;" bgcolor="#FFFFFF">
						...
						<div class="dp"><?php echo $dp[0]." ".$dp[5]?></div>
						
					</td>
                    <td style="width: 250px; " bgcolor="#FFFFFF">
                        <div class="dp">Nombres:</div>
						<div class="dp"><?php echo $dp[1]." ".$dp[6]?></div>
					</td>
				</tr>
				<tr>
                    <td style="width: 110px;padding-left: 20px;" bgcolor="#FFFFFF">
                        <div class="dp">C&eacute;dula:</div>
						<div class="dp"><?php echo $dp[2]?></div>
					</td>
                    <td style="width: 114px;" bgcolor="#FFFFFF">
                        <div class="dp">Expediente:</div>
						<div class="dp"><?php echo $dp[3]?></div>
					</td>
                </tr>
				<tr>
					<td style="width: 570px;padding-left: 20px;" bgcolor="#FFFFFF" colspan=4>
                        <div class="dp">Especialidad: <?php echo $dp[4]?> </div>
					</td>
                </tr>
...
?>
        ...
        <table align="center" border="0" cellpadding="0" cellspacing="1" width="550">
          ...
          <tr><form name="imprime" action="">
               ...
               <td valign="bottom"><p align="left">
                       <input type="button" value="Finalizar" name="bexit" onclick="window.close();"></p> 
                </td></form>
          </tr>
		...
		?>
		  ...
		  <tr  class="dp1">
			<td width="800px">
				<BR><BR><BR><BR>_____________________________________
			</td>
			<td>
				<BR><BR><BR><BR>_____________________________________
			</td>
